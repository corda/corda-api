...
public abstract class ByteSequence implements Comparable<ByteSequence> {
    ...
    ByteSequence(@NotNull byte[] _bytes, int offset, int size) {
        requireNonNull(_bytes, "_bytes must not be null");
        this._bytes = _bytes;
        this.offset = offset;
        this.size = size;
    }
    ...
    @NotNull
    public final ByteSequence subSequence(int offset, int size) {
    ...
    /**
     * Take the first n bytes of this sequence as a sub-sequence.  See {@code subSequence} for further semantics.
     * @param n number of bytes
     * @return byte-sequence
     */
    @NotNull
    public final ByteSequence take(int n) {
    ...
    public final void writeTo(@NotNull OutputStream output) throws IOException {
        requireNonNull(output, "output may not be null");
