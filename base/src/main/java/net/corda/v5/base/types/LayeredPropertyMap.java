 ...
 * <ul>
 * <li>MemberX500Name using {@code parse("corda.name", MemberX500Name::class.java)}</li>
 * <li>session PublicKey using {@code parse("corda.sessionKey", PublicKey::class.java)}</li>
 * <li>list of endpoints using {@code parseList("corda.endpoints", EndpointInfo::class.java)}</li>
 * </ul>
 * Example usages:
 * <ul>
 * <li>Java<pre>{@code
...
public interface LayeredPropertyMap {
     ...
     * @param <T> bleu
     *
     * @throws IllegalArgumentException if the {@code T} is not supported or the {@code key} is blank string.
     ...
     * the {@code T} is different from it was called for the first time.
     *
    ...
    /**
     * Converts the value of the given key to the specified type or returns {@code null} if the key is not found or the value
     * itself is {@code null}.
     *
     * @param key Key for the entry we are looking for.
     * @param clazz The type of the value we want to convert to.
     ...
     * the {@code T} is different from it was called for the first time.
     *
     * @return The parsed values for given type or {@code null} if the key doesn't exist.
     */
    @Nullable
     ...
     * @param <T> bleu
     *
     * @throws IllegalArgumentException if the {@code T} is not supported or the {@code itemKeyPrefix} is blank string.
