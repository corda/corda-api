...
fun ManifestCordappInfo.toAvro(): ManifestCorDappInfo = ManifestCorDappInfo.newBuilder().also {
...
fun Cpi.Metadata.toAvro(timestamp: Instant = Instant.now()): CpiMetadata = CpiMetadata.newBuilder().also { it ->
    it.id = id.toAvro()
    it.hash = SecureHash(hash.algorithm, ByteBuffer.wrap(hash.bytes))
