...
object ConfigKeys {
    ...
    //  RPC
    const val RPC_ADDRESS = "rpc.address"
    const val RPC_CONTEXT_DESCRIPTION = "rpc.context.description"
    const val RPC_CONTEXT_TITLE = "rpc.context.title"
    const val RPC_ENDPOINT_TIMEOUT_MILLIS = "rpc.endpoint.timeoutMs"
    const val RPC_MAX_CONTENT_LENGTH = "rpc.maxContentLength"
    const val RPC_AZUREAD_CLIENT_ID = "rpc.sso.azureAd.clientId"
    const val RPC_AZUREAD_CLIENT_SECRET = "rpc.sso.azureAd.clientSecret"
    const val RPC_AZUREAD_TENANT_ID = "rpc.sso.azureAd.tenantId"
