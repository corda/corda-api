...
class PartialMerkleTree(val root: PartialTree) {
    ...
    companion object {
         ...
         *
         * @throws MerkleTreeException if any of the [includeHashes] are not in the built partial tree.
         */
        @JvmStatic
         ...
         */
        @JvmStatic
        fun rootAndUsedHashes(node: PartialTree, usedHashes: MutableList<SecureHash>, digestService: DigestService): SecureHash {
            return when (node) {
                ...
                is PartialTree.Node -> {
                    val leftHash = rootAndUsedHashes(node.left, usedHashes, digestService)
                    val rightHash = rootAndUsedHashes(node.right, usedHashes, digestService)
                    concatenateAs(digestService, leftHash, rightHash, DigestAlgorithmName(node.hashAlgorithm!!))
                }
            }
        }
        ...
        private fun concatenateAs(
            ...
            val concatBytes = first.bytes + second.bytes
     ...
     */
    fun verify(merkleRootHash: SecureHash, hashesToCheck: List<SecureHash>, digestService: DigestService): Boolean {
        val usedHashes = ArrayList<SecureHash>()
        val verifyRoot = rootAndUsedHashes(root, usedHashes, digestService)
